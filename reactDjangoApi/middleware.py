def dev_cors_middleware(get_response):
    ...
    # intercept the response and pass it each request
    def middleware(request):

        # save the outgoing response object into memory
        response = get_response(request)

        # Mutate the response headers to include all required CORS for local dev environment
